...
# 「30th Celecration 20500円→18500円」のような値下げ告知は価格表ではない。
# 矢印つきの行を商品として拾うと、商品名に価格が混ざったゴミが state に残る。
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        _url = _title = _body = "?"
                    logger.warning("KaitoriExpo: timeline not loaded. url=%s title=%s body=%s",
                                   _url, _title, _body)
                    browser.close()
                    return []

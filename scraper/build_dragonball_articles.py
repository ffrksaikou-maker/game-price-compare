...
HOWTO_ARTICLES = [
    {'slug': 'kougaku-ranking',
     ...
     'meta_desc': 'ドラゴンボールカード(フュージョンワールド・スーパーダイバーズ)の未開封BOX買取価格を、当サイトが6店舗から毎日自動収集した実データで高い順にランキング。1位のMANGA BOOSTER '
     ...
     'hero_sub': '{{TOP1_MULT}}。2位 MANGA BOOSTER 02 {{TOP2_PRICE}}({{TOP2_MULT}})、3位 {{TOP3_NAME}} {{TOP3_PRICE}}({{TOP3_MULT}})。全{{DB_COUNT}}商品の最高買取と定価比を6店舗の実データで毎日更新しています。',
     'body': '<p>ドラゴンボールのカードは、同じ「BOX」でも商品によって買取価格が大きく開きます。定価5,280円前後の通常ブースターが1万円前後で落ち着く一方、<strong>MANGA '
             'BOOSTERは10万円規模</strong>になるのが実情です。本記事では、当サイトが6店舗から毎日自動収集している買取データをもとに、<strong>全{{DB_COUNT}}商品を最高買取価格の高い順にランキング</strong>し、なぜMANGA BOOSTERだけが別格なのかを整理します。</p>\n'
             '\n'
...
<p>予想の土台になるのは、<strong>同じ構成の弾が実際にどの水準にあるか</strong>です。当サイトが6店舗から毎日自動収集している実データでは次のようになっています。</p>
<table class="tbl">
...
<p>売却を考えている場合は、まず <a href="box/sb-01.html">店舗別の買取価格</a> を比較してください。同じBOXでも店舗によって価格差が出ます。シュリンク付きの未開封状態が最も評価されるため、売る可能性があるなら開けないことが前提になります。詳しくは <a href="hatsubai-schedule.html">新弾発売スケジュール</a> で相場が動きやすいタイミングも確認できます。</p>""",
        "faq": [
            ...
            {"q": "SB-01のBOX買取価格はいくらですか?",
             "a": "当サイトが6店舗から収集している実データでは{BOX_PRICE}です(定価{BOX_RETAIL}の{BOX_RATIO})。掲載中のドラゴンボールBOXで最も高い水準ですが、相場は日々変動するため個別ページで最新値をご確認ください。"},
            {"q": "SB-01は開封した方が得ですか?",
...
<p>封入率から見て、特定の高額カードを狙った開封は現実的ではありません。一方でBOX買取は{BOX_PRICE}と定価を大きく上回っています。シュリンク付きの未開封状態が最も高く評価されるため、売る可能性があるなら開けないことが前提です。<a href="box/sb-02.html">店舗別の買取価格</a>を比較してから判断してください。</p>""",
        "faq": [
            ...
            {"q": "SB-02のBOX買取価格はいくらですか?",
             "a": "当サイトが6店舗から収集している実データでは{BOX_PRICE}です(定価{BOX_RETAIL}の{BOX_RATIO})。相場は日々変動するため、個別ページで最新値と店舗ごとの差をご確認ください。"},
        ],
    },
    {
...
<p>売却を考えている場合、シュリンク付きの未開封状態が最も高く評価されます。今後の新弾で相場が動く可能性もあるため、<a href="hatsubai-schedule.html">新弾発売スケジュール</a>もあわせて確認しておくと判断しやすくなります。</p>""",
        "faq": [
            ...
            {"q": "FB-07のBOX買取価格はいくらですか?",
             "a": "当サイトが6店舗から収集している実データでは{BOX_PRICE}です(定価{BOX_RETAIL}の{BOX_RATIO})。この弾は店舗による価格差が大きいため、個別ページで店舗別の価格を必ず比較してください。"},
...
<p>今後の新弾の発売予定は <a href="hatsubai-schedule.html">新弾発売スケジュール</a> にまとめています。新弾の発売前後は既存弾の相場も動きやすいため、あわせて確認しておくと判断材料になります。</p>""",
        "faq": [
            ...
            {"q": "ST-01のBOX買取価格はいくらですか?",
             "a": "当サイトが6店舗から収集している実データでは{BOX_PRICE}です(定価{BOX_RETAIL}の{BOX_RATIO})。2026年8月8日発売と日が浅く相場が動きやすい時期のため、個別ページで最新値をご確認ください。"},
...
<p>当サイトの実データでのBOX買取は{BOX_PRICE}({BOX_RATIO})です。シュリンク付きの未開封状態が最も高く評価されるため、売る可能性があるなら開けないことが前提になります。<a href="box/fb-09.html">FB-09の個別ページ</a>で店舗別の価格と価格推移を確認してください。</p>""",
        "faq": [
            ...
            {"q": "FB-09のBOX買取価格はいくらですか?",
             "a": "当サイトが6店舗から収集している実データでは{BOX_PRICE}です(定価{BOX_RETAIL}の{BOX_RATIO})。相場は日々変動するため、個別ページで最新値をご確認ください。"},
        ],
    },
    {
...
<p>シュリンク付きの未開封状態が最も高く評価されます。<a href="box/fb-02.html">FB-02の個別ページ</a>で店舗別の買取価格を比較してから判断してください。</p>""",
        "faq": [
            ...
            {"q": "FB-02のBOX買取価格はいくらですか?",
             "a": "当サイトが6店舗から収集している実データでは{BOX_PRICE}です(定価{BOX_RETAIL}の{BOX_RATIO})。トップレアが約5万円である一方でBOX買取が低めなのは、カード1枚の価格とBOX価格が連動しないためです。"},
        ],
    },
    {'slug': 'fb-11-atari-guide',
     ...
     'body': '<p>2026年9月12日(土)発売のドラゴンボールスーパーカードゲーム フュージョンワールド ブースターパック第11弾 <strong>「BRIGHTNESS OF HOPE」【FB-11】</strong> '
             ...
             '<li><strong>BOX買取価格</strong>: 当サイトが6店舗から毎日自動収集します。最高値の店舗は <a href="/dragonball">比較トップ</a> で確認できます</li>\n'
     ...
     'faq': [{'q': 'FB-11「BRIGHTNESS OF HOPE」の当たりカードは何ですか?',
             ...
             {'q': 'FB-11のBOX買取価格はいくらですか?', 'a': '発売前のため買取価格はまだ付いていません。発売後は当サイトが6店舗から毎日自動収集し、ドラゴンボールBOX買取価格比較トップで最高値の店舗を確認できるようにします。'}]},
]
...
<div class="header"><a href="/dragonball"><span class="site-title">{SITE_NAME}</span></a></div>
